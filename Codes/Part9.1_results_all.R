...
for(i in 1:length(Disease)){
  ...
  SNP_lower_region <- SNPs$Location - 5000
  SNP_upper_region <- SNPs$Location + 5000
  SNP_region <- data.frame(SNP = SNPs$SNP.rs, Chr = SNPs$Chromosome, lower = SNP_lower_region, upper = SNP_upper_region)
  
  save(SNP_region,file = paste0(wd, Di, "/SNP_region.rda"))
  ...
  write.table(SNP_region2, paste0(wd, Di, "/SNP_region2.txt"), append = F, row.names = F, col.names = F)
...
for(i in 1:length(Disease)){
  Di <- Disease[i]
  SNP_10kb <- read.delim(paste0(wd,Di,"/SNP-10kb.txt"),header = T)
  save(SNP_10kb, file = paste0(wd,Di,"/SNP_10kb.rda"))
  ...
  save(SNP_10kb_mut2, file = paste0(wd, Di, "/SNP_10kb_mut2.rda"))
  
  SNP_10kb_mut2_23 = SNP_10kb_mut2[which(SNP_10kb_mut2$X.chrom %in% paste0("chr",c(1:22,"X"))),]
  save(SNP_10kb_mut2_23, file = paste0(wd, Di, "/SNP_10kb_mut2_23.rda"))
...
for(j in 1:length(Disease)){
  ...
  for (i in 1:23){
    ...
    write.table(target.snp_2, paste0(wd, Di, "/target.snp_2.txt"), append = T, row.names = F,col.names = F)
  }
  
  save(target.snp, file = paste0(wd, Di, "/target.snp.rda"))
  
  target.snp_2 <- read.table(paste0(wd,Di,"/target.snp_2.txt"), header = F)
  colnames(target.snp_2) <- c("seq", "chr", "position","name")
  save(target.snp_2, file = paste0(wd, Di, "/target.snp_2.rda"))
...
for(i in 1:length(Disease)){
  Di <- Disease[i]
  target.snp <- get(load(paste0(wd, Di, "/target.snp.rda")))
  target.snp_2 <- get(load(paste0(wd, Di, "/target.snp_2.rda")))
  SNP_10kb_mut2_23 <- get(load(paste0(wd, Di, "/SNP_10kb_mut2_23.rda")))
  ...
  save(seq.ref,file = paste0(wd, Di, "/seq.ref.rda"))
  save(seq.snp,file = paste0(wd, Di, "/seq.snp.rda"))
}
...
get.quantile <- function(ii){
  ...
  write.table(thre, paste0(wd,"quantile.new2.txt"), append = T, row.names = F,col.names = F)
}

for(ii in 1:length(name)){
  get.quantile(ii)
}
quan = read.table(paste0(wd,"quantile.new2.txt"), header = F)
...
results <- function(Disease){
  ...
  for(i in 1:length(Disease)){
    ...
    for(j in 1:length(name)){
      ...
      save(result, file = paste0(wd, Di, "/result", TF, ".rda"))
...
for(i in 1:length(Disease)){
  ...
  for(j in 1:length(name)){
    ...
    save(gkmSVM.ref, file = paste0(wd,Di, "/gkmSVM.ref", TF, ".rda"))
    save(gkmSVM.snp, file = paste0(wd,Di, "/gkmSVM.snp", TF, ".rda"))
    
    deltaSVM = gkmSVM.snp - gkmSVM.ref
    
    result <- cbind(result[,1:5], deltaSVM)
    result$sig_SNPs <- ifelse(deltaSVM < quan$left[j] | deltaSVM > quan$right[j], 1 , 0)
    save(result, file = paste0(wd, Di, "/result", TF, ".rda"))
    write.table(result, paste0(wd, Di, "/result_", TF, ".txt"), append = F, row.names = F,col.names = F)
  }
}
...
for(i in 1:length(Disease)){
  ...
  for(j in 1:length(name)){
    ...
    result_motifs <- get(load(paste0(wd, Di, "/result", TF, ".rda")))
    ...
    save(result_motifs, file = paste0(wd, Di, "/result_motifs", TF, ".rda"))
  }
}
...
for(i in 1:length(Disease)){
  Di <- Disease[i]
  final_result = read.table(paste0(wd, Di, "/result.txt"), header = F)
  colnames(final_result) = c("snp", "chr", "position", "Seqref", "Seqmut", "DeltaSVM", "significant SNP")
  write.csv(final_result, paste0(wd, Di, "/result.csv"), row.names = F)
  write_xlsx(final_result,path = paste0(wd, Di, "/result.xlsx"))
...
for (i in 1:length(Disease)) {
  ...
  for(j in 1:length(name)){
    TF <- name[j]
    wd2 <- paste0(wd, Di, "/result", TF, ".rda")
    if(!file.exists(wd2)) {
      next
    }
    
    result.TF <- get(load(paste0(wd, Di, "/result", TF, ".rda")))
    
    result_1st <- cbind(nrow(result.TF),result.TF[1,])
    colnames(result_1st) <- c("nrow", colnames(result.TF))
    write.table(result_1st, paste0(wd, Di, "/result_1st.txt"), append = T, row.names = F,col.names = F)
  }
...
for(i in 1:length(Disease)){
  ...
  resultBCL11A <- get(load(paste0(wd, Di, "/resultBCL11A.rda")))
  resultCTCF <- get(load(paste0(wd, Di, "/resultCTCF.rda")))
  resultEGR1 <- get(load(paste0(wd, Di, "/resultEGR1.rda")))
  resultGABPA <- get(load(paste0(wd, Di, "/resultGABPA.rda")))
  resultJUND <- get(load(paste0(wd, Di, "/resultJUND.rda")))
  resultJUN <- get(load(paste0(wd, Di, "/resultJUN.rda")))
  resultMAX <- get(load(paste0(wd, Di, "/resultMAX.rda")))
  resultNANOG <- get(load(paste0(wd, Di, "/resultNANOG.rda")))
  resultPOU5F1 <- get(load(paste0(wd, Di, "/resultPOU5F1.rda")))
  resultRAD21 <- get(load(paste0(wd, Di, "/resultRAD21.rda")))
  
  f1 <- paste0(wd, Di, "/resultREST.rda")
  
  if(file.exists(f1)){
    resultREST <- get(load(paste0(wd, Di, "/resultREST.rda")))
  }
  
  resultRFX5 <- get(load(paste0(wd, Di, "/resultRFX5.rda")))
  resultSIX5 <- get(load(paste0(wd, Di, "/resultSIX5.rda")))
  resultSRF <- get(load(paste0(wd, Di, "/resultSRF.rda")))
  resultSTAT1 <- get(load(paste0(wd, Di, "/resultSTAT1.rda")))
  resultTCF12 <- get(load(paste0(wd, Di, "/resultTCF12.rda")))
  resultUSF1 <- get(load(paste0(wd, Di, "/resultUSF1.rda")))
  resultUSF2 <- get(load(paste0(wd, Di, "/resultUSF2.rda")))
  resultYY1 <- get(load(paste0(wd, Di, "/resultYY1.rda")))
  
  if(file.exists(f1)){
    result_motif <- rbind(resultBCL11A,
                          NA,
                          resultCTCF,
                          NA,
                          resultEGR1,
  ...
  write.csv(result_motif, file = paste0(wd, Di, "/result_motif.csv"), na ="")
